module Tests exposing (all, fuzz5, graphTests)
...
fuzz5 fa fb fc fd fe desc fn =
    fuzz3 (tuple ( fa, fb )) (tuple ( fc, fd )) fe desc (\( a, b ) ( c, d ) e -> fn a b c d e)
                ...
                \a b c d e ->
                    allDifferent [ a, b, c, d, e ] <|
                        let
                            ...
                            func =
                                \key maybeData -> maybeData |> Maybe.map (\x -> x + 1)
                        in
                        graph
                            |> map func
                            |> nodes
            ...
            , test "topSort doesn't return an ordering if there are cycles" <|
                \() ->
                    ...
                    case graph |> topologicalSort of
                        ...
                        Just order ->
                            Expect.fail "Got an ordering despite input being cyclical"
            , test "topSort doesn't return an ordering if there are loops" <|
                \() ->
                    ...
                    case graph |> topologicalSort of
                        ...
                        Just order ->
                            Expect.fail "Got an ordering despite input having loops"
            ]
        , describe "isAcyclic"
