module Graph exposing
    ( Graph
    , getData, getEdgeData, member, memberEdge, incoming, outgoing, isEmpty, size, keys, nodes, edges, edgesWithData, isAcyclic
    , empty, insert, insertData, insertEdge, insertEdgeData, remove, removeData, removeEdge, removeEdgeData, update, updateEdge
    , map, mapEdge, foldl, foldr
    , filter, partition, union, intersect
    , postOrder, topologicalSort
    )
...
emptyNode : Node comparable data edgeData
emptyNode =
    Node
        { data = Nothing
        , incoming = Set.empty
...
nodeWithData : data -> Node comparable data edgeData
nodeWithData data =
    Node
        { data = Just data
        , incoming = Set.empty
...
getOrCreate key graph =
    get key graph |> Maybe.withDefault emptyNode
...
remove key (Graph graph) =
    case get key (Graph graph) of
        ...
        Just (Node node) ->
            ...
            in
            List.foldl (\( from, to ) -> removeEdge from to) newGraph (incomingEdgesToRemove ++ outgoingEdgesToRemove)
...
isAcyclicHelper topSortedNodes seen graph =
    case topSortedNodes of
        ...
        k :: ks ->
            if Set.member k (incoming k graph) then
                -- has an edge to itself
                False

            else if not <| Set.isEmpty <| Set.intersect seen (incoming k graph) then
            ...
            Dict.map
                (\from (Node node) ->
                    Node
                        { data = node.data
                        , incoming = node.incoming
                        , outgoing = Dict.map (func from) node.outgoing
                        , reachable = node.reachable
                        }
                )
                graph.nodes
    ...
    let
        ...
        removeDeadEdges =
            \key (Node node) ->
                Node
                    { node
                        | incoming = node.incoming |> Set.filter (\k -> get k (Graph graph) /= Nothing)
                        , outgoing = node.outgoing |> Dict.filter (\k _ -> get k (Graph graph) /= Nothing)
                    }
    in
...
reversePostOrderHelper nodeKeys keyOrder seenKeys graph =
    case nodeKeys of
        ...
        k :: ks ->
            if Set.member k seenKeys then
                reversePostOrderHelper ks keyOrder seenKeys graph

            else
                let
                    ( seen, order ) =
                        reversePostOrderHelper
                            (outgoing k graph |> Set.toList)
                            keyOrder
                            (Set.insert k seenKeys)
                            graph
                in
                reversePostOrderHelper ks (k :: order) seen graph



{- # Debugging functions; unpublishable because we're using `Debug.toString`.
   ...
   valid (Graph graph) =
       ...
       let
           hasDanglingOutgoingEdges k =
               outgoing k (Graph graph) |> Set.toList |> List.map (\key -> incoming key (Graph graph)) |> List.all (Set.member k) |> not

           hasDanglingIncomingEdges k =
               incoming k (Graph graph) |> Set.toList |> List.map (\key -> outgoing key (Graph graph)) |> List.all (Set.member k) |> not
           ...
           reachabilityCacheIsStale k =
               (outgoing k (Graph graph) /= Set.empty)
                   && (reachable k
                           /= (List.foldl
                                   Set.union
                                   (outgoing k (Graph graph))
                               <|
                                   List.map (\key -> reachable key) <|
                                       (Set.toList <| outgoing k (Graph graph))
                              )
                      )
       in
       if keys (Graph graph) |> List.any hasDanglingOutgoingEdges then
           Err (Debug.toString ( "found dangling outgoing edge", Graph graph ))

       else if keys (Graph graph) |> List.any hasDanglingIncomingEdges then
           Err (Debug.toString ( "found dangling incoming edge", Graph graph ))
